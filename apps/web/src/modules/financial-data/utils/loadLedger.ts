...
export async function loadAtlasLedger(userId: string): Promise<LedgerBundle> {
  ...
  const [txRes, billsRes, goalsRes, profileRes, fixedRes] = await Promise.all([
    ...
    listBills(userId).then(
      (data) => ({ ok: true as const, data }),
      (erro: unknown) => {
        errors.bills = getFriendlyErrorMessage(erro, "Não foi possível carregar suas contas.");
    ...
    listGoals(userId).then(
      (data) => ({ ok: true as const, data }),
      (erro: unknown) => {
        errors.goals = getFriendlyErrorMessage(erro, "Não foi possível carregar suas metas.");
